...
# 1. ddsDESeqObject(DESeqDataSet) the output of gene expression analysis from DESeq
...
getFPKMs <- function(ddsDESeqObject, ebg, verbose = FALSE) {
  ...
  #    number of basepairs in the union of all GRanges assigned to a given row of object,
  #    e.g., the union of all basepairs of exons of a given gene.
  # feature length is used to calculate FPKM: fragments per kilobase per million mapped fragments
  mat.norm <- DESeq2::fpkm(ddsDESeqObject, robust=T)
  # mat.norm is a matrix of the  genes x samples but now normalized
  if (verbose) cat("now dimention of fpkm normalized matrix is ", dim(mat.norm))
  return(mat.norm)
}
...
getDEgenes <- function(ddsDESeqObject,padj_max = 0.01,lfc_min = 2, verbose = FALSE){
  ...
  #     due to treatment with dexamethasone in comparison to untreated samples. This value is
  #     reported on a logarithmic scale to base 2: for example, a log2 fold change of 1.5 means that the
  #     gene's expression is increased by a multiplicative factor of 2^1.5≈2.82."
  #
  #     here we use minimum 2 which means only genes that are four times higher or lower in the treatment group are selected
  #     (log2(4) = 2; 2^2 = 4)
  #
  #2. pvalue
  #     p value indicates the probability that a fold change as strong as the observed one,
  #     or even stronger, would be seen under the situation described by the null hypothesis.
  ...
  # move rownames to listData(just a list of results)  inside res.sel(DESeqResults)
...
getFPKMFCs <- function(mat.norm, ddsDESeqObject, verbose = FALSE) {
  ...
  if (verbose) cat("\n the input matrix dimentions was ",dim(mat.norm),"\n mat.ni is ",dim(mat.ni),
                   "\n mat.i is ",dim(mat.i), "and the output mat.fcs is", dim(mat.fcs) )
  return(mat.fcs)
}
